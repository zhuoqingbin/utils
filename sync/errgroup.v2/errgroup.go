...
func (g *Group) doWithContext(ctx context.Context, f func(ctx context.Context) error) {
	//ctx := g.ctx
	//if ctx == nil {
	//	ctx = context.Background()
	//}
	//ctx = context.WithCancel(_ctx)
	var err error
	defer func() {
		if r := recover(); r != nil {
			buf := make([]byte, 64<<10)
...
// The first call to return a non-nil error cancels the group; its error will be
// returned by Wait.
func (g *Group) GoWithContext(ctx context.Context, f func(ctx context.Context) error) {
	g.wg.Add(1)
	if g.ch != nil {
		select {
		case g.ch <- f:
