...
// FromHTTPProxy returns HTTP tunnel dialer proxying tcp connection.
// Copy from https://gist.github.com/jim3ma/3750675f141669ac4702bc9deaf31c6b
// Usage:
// dialer, err := DialWithHTTPProxy("http://your http proxy:3128")
// lg.PanicError(err)
...
func FromHTTPProxy(proxyURI string) (proxy.Dialer, error) {
	httpProxyURI, err := url.Parse(proxyURI)
	if err != nil {
		return nil, err
	}
	return proxy.FromURL(httpProxyURI, Direct)
}
